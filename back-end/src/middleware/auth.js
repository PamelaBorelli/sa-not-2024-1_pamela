...
export default function(req, res, next) {
  ...
  */
  ...
  /* PROCESSO DE VERIFICAÇÂO DO TOKEN DE AUTENTICAÇÃO */
  ...
  // 2. Se o cokkie contendo o token não existir, procuramos no cabeçalho de autorização
  if(! token){

    // token pode ter sido enviado no cabeçalho "autorization"
    const authHeader = req.headers['authorization']

    // O token não foi encontrado nem no header -> HTTP 403: Forbiden
